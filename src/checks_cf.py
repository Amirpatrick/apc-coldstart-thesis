...
def train_cf(mat: sp.csr_matrix) -> np.ndarray:
    """Train an implicit-ALS teacher; fall back to truncated SVD if unavailable."""
    try:
        ...
        print(f"  implicit ALS (factors={C.CF_FACTORS}, gpu={bool(HAS_CUDA)})")
        ...
        print("  implicit not installed -> falling back to TruncatedSVD "
              "(weaker teacher; install `implicit` for the real thing)")
...
def group_cohesion(
    ...
    for g in keys:
        ...
        # size-matched random null: |n-1| random warm items per member
        rnd_idx = rng.integers(0, n_items, size=(n, n - 1))
        rnd_proto = _l2(factors_n[rnd_idx].mean(axis=1))
        null = np.einsum("ij,ij->i", E, rnd_proto)
...
def check2(force_cf: bool = False) -> None:
    ...
    for level in C.GROUP_LEVELS:
        ...
        g = g[g["song_id"].isin(row_of.keys())]
        if len(g) == 0:
            continue
