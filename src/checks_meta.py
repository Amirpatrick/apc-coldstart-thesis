...
def attach_appearances(songs: pd.DataFrame, inter: pd.DataFrame) -> pd.DataFrame:
    """Join playlist-appearance counts onto the song table.

    Both keys are coerced to int64 first. A dtype mismatch here is the single
    most likely silent failure in the pipeline (an unrecognised id field turns
    the column into all-None object dtype), so we check it explicitly.
    """
    songs = songs.copy()
    if songs["song_id"].isna().all():
        raise ValueError(
            "songs['song_id'] is entirely null -- the id field was not parsed. "
            "Rebuild the cache: python run_checks.py build --force"
        )
...
def check1() -> None:
    ...
    # -- 1a. What is the catalogue, actually? ------------------------------
    n_records = len(songs)
    try:
        ref = prepare.load_referenced()
        ref_ids = set(ref.loc[ref["referenced_anywhere"], "song_id"].astype("int64"))
        n_ref = len(ref_ids)
        songs["referenced"] = songs["song_id"].isin(ref_ids)
    except FileNotFoundError:
        n_ref = None
        songs["referenced"] = True

    degenerate = (
        (songs["n_artists"] == 0)
        | songs["song_name"].isna()
        | (songs["song_name"].astype(str).str.strip() == "")
    )

    r.heading("1a. Defining the catalogue (this must be settled first)")
    r.text(
        "The documentation states 649,091 unique tracks over an ID space of "
        "0-707,988 'with gaps'. The file we actually hold carries one record "
        "per ID across that whole range, so there are no gaps and the record "
        "count is not the track count. Every share reported later depends on "
        "which of these is used as the denominator, so we fix it here."
    )
    kv = {
        "Records in song_meta.json": f"{n_records:,}",
        "Documented unique tracks": "649,091",
        "Records with no artist or no title": f"{int(degenerate.sum()):,} ({degenerate.mean():.1%})",
    }
    if n_ref is not None:
        kv["Referenced by >=1 playlist (train+val+test)"] = f"{n_ref:,}"
        kv["Referenced by >=1 TRAIN playlist"] = f"{int((songs['appearances'] > 0).sum()):,}"
        kv["In metadata but never referenced"] = (
            f"{int((~songs['referenced']).sum()):,} "
            f"({(~songs['referenced']).mean():.1%})"
        )
    r.kv(kv)
    r.text(
        "**Decision.** The working catalogue is the set of metadata records "
        "referenced by at least one playlist in any official split. Records "
        "that no playlist ever mentions cannot be evaluated (they have no "
        "ground truth in any protocol) and are excluded from all denominators. "
        "This is stated as a preprocessing rule, applied identically to every "
        "method, and reported here rather than buried."
    )

    songs_all = songs
    songs = songs[songs["referenced"]].copy()
    r.kv({"Working catalogue size": f"{len(songs):,}"})

    # -- 1b. Warm/cold vocabulary -----------------------------------------
    n_tracks = len(songs)
    n_warm = int(songs["is_warm"].sum())
    n_zero = int((songs["appearances"] == 0).sum())
    n_le1 = int((songs["appearances"] <= 1).sum())
    r.heading("1b. Warm / cold split of the working catalogue")
    r.kv(
        {
            "Tracks in working catalogue": f"{n_tracks:,}",
            f"Warm (>= {C.WARM_MIN_APPEARANCES} appearances in train)": f"{n_warm:,} ({n_warm/n_tracks:.1%})",
            "Cold (below threshold)": f"{n_tracks-n_warm:,} ({1-n_warm/n_tracks:.1%})",
            "Never in any TRAIN playlist": f"{n_zero:,} ({n_zero/n_tracks:.1%})",
            "In 0 or 1 train playlists": f"{n_le1:,} ({n_le1/n_tracks:.1%})",
        }
    )
    r.text(
        "Ferraro et al. report 81,219 warm tracks. Our count is computed from "
        "`train.json` only, because the val/test ground truth is private and "
        "Kakao Arena is unavailable — that is the split we must build on "
        "regardless. Tracks appearing only in val/test playlists therefore "
        "show 0 train appearances and are correctly counted as cold."
    )

    # -- 1b. Catalog size distribution, two artist definitions -------------
    r.heading("1c. Tracks per artist")
    ...
    r.heading("1d. Warm-catalog size buckets (this is the decisive table)", 3)
    r.text(
    ...
    r.heading("1e. Per cold *track*: how much artist context is available?")
    r.text(
